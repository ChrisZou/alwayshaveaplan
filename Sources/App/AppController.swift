...
final class AppController: NSObject, ObservableObject {
    ...
    override init() {
        ...
        scheduleUnlockCheck(reason: "initialLaunch", openCalendarIfNoEvents: true, showEventsIfAny: true)
        calendarManager.requestAccessIfNeeded { [weak self] _ in
            self?.scheduleUnlockCheck(reason: "calendarPermissionChanged", openCalendarIfNoEvents: true, showEventsIfAny: true)
        }
    }
    ...
    private func setupUnlockObservers() {
        ...
        center.publisher(for: NSWorkspace.sessionDidBecomeActiveNotification)
            .sink { [weak self] _ in
                ...
                self?.scheduleUnlockCheck(reason: "sessionDidBecomeActive", openCalendarIfNoEvents: true, showEventsIfAny: true)
            }
        ...
        center.publisher(for: NSWorkspace.screensDidWakeNotification)
            .sink { [weak self] _ in
                ...
                self?.scheduleUnlockCheck(reason: "screensDidWake", openCalendarIfNoEvents: true, showEventsIfAny: true)
            }
            .store(in: &cancellables)
        ...
        if #available(macOS 13.0, *) {
            center.publisher(for: NSWorkspace.didWakeNotification)
                .sink { [weak self] _ in
                    ...
                    self?.scheduleUnlockCheck(reason: "didWake", openCalendarIfNoEvents: true, showEventsIfAny: true)
                }
                .store(in: &cancellables)
        }
        ...
        distributedObservers.append(
            dnc.addObserver(forName: Notification.Name("com.apple.screenIsUnlocked"), object: nil, queue: .main) { [weak self] _ in
                ...
                self?.scheduleUnlockCheck(reason: "screenIsUnlocked", openCalendarIfNoEvents: true, showEventsIfAny: true)
            }
        )
        distributedObservers.append(
    ...
    private func startPeriodicChecks() {
        ...
        periodicTimer = Timer.scheduledTimer(withTimeInterval: 60, repeats: true) { [weak self] _ in
            ...
            self?.scheduleUnlockCheck(reason: "periodicTimer", openCalendarIfNoEvents: false, showEventsIfAny: false)
        }
        RunLoop.main.add(periodicTimer!, forMode: .common)
    }

    private func scheduleUnlockCheck(reason: String, openCalendarIfNoEvents: Bool, showEventsIfAny: Bool) {
    ...
    private func performCheck(reason: String, openCalendarIfNoEvents: Bool, exitOnEvent: Bool, showEventsIfAny: Bool) {
